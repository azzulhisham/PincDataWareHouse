...
def main():
    ...
    for i in data:
        if i != '':
            ...
            if totalPackage == '1':
                if msgType == '6' :     #msgType == '1' or msgType == '2' or msgType == '3'
                    tagblock = i[:i.index('!')]
                    tags = tagblock.split(',')
                    tagtime = tags[2]
                    tagtime = tagtime[2:tagtime.index('*')]
    ...
    posdataset = pd.DataFrame.from_dict(aisdatalist)
    ...
    dataset = pd.merge_asof(posdataset, staticdataset, on="ts", by="mmsi")
    dataset.to_parquet(f'/Users/zultan/Downloads/pyais_gold{filedate}.parquet', index=False)
    print(dataset.shape)
